...
<body>
    ...
    <?php if (count($usecase->result()) == 0) { ?>
        <div id="notfound" class="container-fluid p-4">
            <div class="notfound">
                <br>
                <br><br><br>
                <br><br>
                <h2>We are sorry, Maybe you are not <br> Use Case Member</h2>
    ...
    <?php } else { ?>
        ...
        <div class="container p-4" id="bg-template">
            ...
            <p> Please fill Daily Timesheet (DT) according to your use case and submit approval to your supervisor. DT status should be approved by supervisor.</p>
                    ...
                    <!-- <div class="md-12 mb-4">
                        ...
                        <select class="form-control text-uppercase" style="width:auto;" id="usecase" onchange="changeDatatable()">
                            < ?php foreach ($usecase->result() as $usecase) { ?>
                                < ?php if($usecase->id_usecase == $usecase_default->id_usecase){ ?>
                                <option value="< ?= $usecase->id_usecase ?>" selected>< ?= $usecase->nama_usecase ?></option>
                                < ?php } else { ?>
                ...
                <tbody id="datatable">
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($use_task->status == 1 or $use_task->status == 4) { ?>
                                ...
                                <?php } else { ?>
                                    <div class="btn-group-vertical">
                                        ...
                                        <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#deleteUseCaseTask/<?= $use_task->id_usecase_task ?>/<?= $use_task->id_usecase ?>" disabled><em class="fas fa-trash"></em> Delete</button>
                                    </div>
                                <?php } ?>
                            </td>
